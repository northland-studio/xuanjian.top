    ...
    try {
        if (channel === 'dm') {
            ...
            const r = await db.run(
                `INSERT INTO chat_messages (channel, sender_id, receiver_id, content, bubble_id) VALUES ('dm', ?, ?, ?, ?)`,
                [userId, to, content, bubbleId]
            );
            ...
            const r = await db.run(
                `INSERT INTO chat_messages (channel, sender_id, receiver_id, content, bubble_id) VALUES ('public', ?, NULL, ?, ?)`,
                [userId, content, bubbleId]
            );
            const row = await db.get('SELECT * FROM chat_messages WHERE id=?', [r.id]);
            const [dto] = await enrich([row]);
...
module.exports = { handleMessage, publicHistory, dmHistory, broadcastAll, sendToUsers, onlineCount, enrich };
