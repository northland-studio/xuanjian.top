...
async function enrich(rows) {
    ...
    // 引用回复：先取出被引用的原消息，并把其发送者一并纳入用户查询
    const replyIds = [...new Set(rows.map(r => r.reply_to_id).filter(Boolean))];
    let rmap = new Map();
    if (replyIds.length) {
        const rph = replyIds.map(() => '?').join(',');
        const rs = await db.all(
            `SELECT id, sender_id, content, image_url, sticker_url, voice_url FROM chat_messages WHERE id IN (${rph})`,
            replyIds
        );
        rmap = new Map(rs.map(x => [x.id, x]));
        rs.forEach(x => { if (x.sender_id && !ids.includes(x.sender_id)) ids.push(x.sender_id); });
    }
    ...
    const previewOf = (x) => {
        if (!x) return '';
        if (x.image_url) return '[图片]';
        if (x.sticker_url) return '[表情]';
        if (x.voice_url) return '[语音]';
        return String(x.content || '').slice(0, 60);
    };
    return rows.map(r => {
        ...
        const ref = r.reply_to_id ? rmap.get(r.reply_to_id) : null;
        const refUser = ref ? (umap.get(ref.sender_id) || {}) : {};
        return {
            id: r.id,
            channel: r.channel,
            content: r.content,
            ...
            replyTo: ref ? {
                id: ref.id,
                senderId: ref.sender_id,
                senderName: refUser.nickname || refUser.username || '用户',
                preview: previewOf(ref),
            } : null,
        };
    });
}
...
async function dmConversations(userId) {
    ...
    // 每个会话的未读数（对方发给我、且我尚未读的）
    const unreadRows = await db.all(
        `SELECT sender_id, COUNT(*) AS c FROM chat_messages
         WHERE channel='dm' AND receiver_id=? AND read_at IS NULL
           AND (expires_at IS NULL OR expires_at > ?)
         GROUP BY sender_id`,
        [userId, now]
    );
    const unreadMap = new Map(unreadRows.map(r => [r.sender_id, r.c]));
    ...
    for (const [other, lastMsg] of seen) {
        ...
        out.push({
            ...
            // 最后一条是否我发的、以及对方是否已读（用于会话列表展示送达状态）
            lastFromMe: lastMsg.sender_id === userId,
            lastRead: !!lastMsg.read_at,
            unread: unreadMap.get(other) || 0,
        });
    }
    // 有未读的会话排前面，其余按最后消息时间倒序
    out.sort((a, b) => (b.unread - a.unread) || String(b.lastAt).localeCompare(String(a.lastAt)));
    return out;
}

/**
 * 把「某人发给我的私聊」全部标记为已读。
 * @returns {Promise<{count:number, ids:number[], readAt:string|null}>}
 */
async function markDmRead(userId, otherId) {
    const now = utcStr(Date.now());
    const rows = await db.all(
        `SELECT id FROM chat_messages
         WHERE channel='dm' AND sender_id=? AND receiver_id=? AND read_at IS NULL
           AND (expires_at IS NULL OR expires_at > ?)`,
        [otherId, userId, now]
    );
    if (!rows.length) return { count: 0, ids: [], readAt: null };
    const ids = rows.map(r => r.id);
    await db.run(
        `UPDATE chat_messages SET read_at=? WHERE id IN (${ids.map(() => '?').join(',')})`,
        [now, ...ids]
    );
    return { count: ids.length, ids, readAt: now };
}

/** 我在各会话中的未读总数（用于红点/角标） */
async function dmUnreadTotal(userId) {
    const now = utcStr(Date.now());
...
async function handleMessage(ws, raw) {
    ...
    // 已读回执：把对方发给我的消息标记为已读，并通知对方（双方都收到，便于多端同步）
    if (m.type === 'chat_read') {
        const other = parseInt(m.with);
        if (!other) return;
        try {
            const r = await markDmRead(userId, other);
            if (r.count) {
                sendToUsers([userId, other], {
                    type: 'chat_read',
                    channel: 'dm',
                    by: userId,
                    with: other,
            ...
            logger.error('私聊已读回执处理失败:', e.message);
        }
        return;
    }
    ...
    try {
        if (channel === 'dm') {
            ...
            // 回复引用：必须引用同一会话内的消息，防止跨会话引用
            let replyToId = m.replyTo ? parseInt(m.replyTo) : null;
            if (replyToId) {
                const ref = await db.get(
                    `SELECT id FROM chat_messages
                     WHERE id=? AND channel='dm'
                       AND ((sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?))`,
                    [replyToId, userId, to, to, userId]
                );
                if (!ref) replyToId = null;
            }

            const r = await db.run(
                `INSERT INTO chat_messages (channel, sender_id, receiver_id, content, bubble_id, image_url, sticker_url, voice_url, voice_duration, expires_at, reply_to_id)
                 VALUES ('dm', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [userId, to, content, bubbleId, imageUrl, stickerUrl, voiceUrl, voiceDuration, expiresAt, replyToId]
            );
...
module.exports = {
    handleMessage, publicHistory, dmHistory, dmConversations, markDmRead, dmUnreadTotal,
    cleanupExpired, cleanupIsEmpty, broadcastAll, sendToUsers, onlineCount, enrich,
    dmMediaDays, publicMediaDays,
};
