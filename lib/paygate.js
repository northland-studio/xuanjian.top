...
async function handleOrder(orderNo) {
    ...
    try {
        ...
        // 缴费单（需用户确认，有 confirm_token）与 外站兑换 区分流水类型
        const isPayment = !!order.confirm_token;
        const logType = isPayment ? 'payment' : 'exchange';
        const note = order.direction === 'out'
            ? (isPayment ? `缴费单支出 ${order.amount}（${order.site_order_no}）` : `外站兑换支出 ${order.amount}（${order.site_order_no}）`)
            : (isPayment ? `缴费单充入 ${order.amount}（${order.site_order_no}）` : `外站充入/兑换 ${order.amount}（${order.site_order_no}）`);
        await addContributionLog(order.user_id, finalAmount, logType, order.id, note);
