...
function initLazyLoad() {
    ...
    if ('IntersectionObserver' in window) {
        ...
        // 不支持 IntersectionObserver 的浏览器回退
        document.querySelectorAll('img[data-src]').forEach(img => {
            img.src = img.dataset.src;
        });
        document.querySelectorAll('img[data-bg]').forEach(img => {
            img.style.backgroundImage = `url(${img.dataset.bg})`;
        });
    }
}

// 动态添加懒加载图片（用于 AJAX 加载的内容）
function observeNewImages(container) {
    if (!container) return;
    
    container.querySelectorAll('img[data-src], img[data-bg]').forEach(img => {
        if ('IntersectionObserver' in window) {
            const imageObserver = new IntersectionObserver((entries, observer) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
